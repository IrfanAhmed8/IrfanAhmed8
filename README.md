# Hi 👋 I'm Irfan Ahmed Jafri

🚀 Full Stack Developer | | Distributed Systems Enthusiast
💡 Building scalable systems & real-time applications
📍 Mumbai, India

---

## 🧠 About Me

* 🎓 Computer Science Undergraduate (CGPA: 8.56)
* 💻 Solved **300+ DSA problems** on LeetCode
* ⚡ Strong in **Backend Engineering & System Design**
* 🔥 Interested in **high-performance systems & quantitative infrastructure**

---

## 🛠️ Tech Stack

### 💻 Languages

![Java](https://img.shields.io/badge/Java-orange?style=for-the-badge\&logo=openjdk)
![Python](https://img.shields.io/badge/Python-blue?style=for-the-badge\&logo=python)
![C++](https://img.shields.io/badge/C++-blue?style=for-the-badge\&logo=cplusplus)
![JavaScript](https://img.shields.io/badge/JavaScript-yellow?style=for-the-badge\&logo=javascript)

### ⚙️ Backend

![Spring Boot](https://img.shields.io/badge/SpringBoot-green?style=for-the-badge\&logo=springboot)
![Node.js](https://img.shields.io/badge/Node.js-green?style=for-the-badge\&logo=node.js)
![Express](https://img.shields.io/badge/Express-black?style=for-the-badge\&logo=express)
![Microservices](https://img.shields.io/badge/Microservices-grey?style=for-the-badge)

### 🎨 Frontend

![React](https://img.shields.io/badge/React-blue?style=for-the-badge\&logo=react)
![Tailwind](https://img.shields.io/badge/TailwindCSS-blue?style=for-the-badge\&logo=tailwindcss)

### 🗄️ Databases

![MySQL](https://img.shields.io/badge/MySQL-blue?style=for-the-badge\&logo=mysql)
![MongoDB](https://img.shields.io/badge/MongoDB-green?style=for-the-badge\&logo=mongodb)

### ⚡ DevOps & Tools

![Docker](https://img.shields.io/badge/Docker-blue?style=for-the-badge\&logo=docker)
![AWS](https://img.shields.io/badge/AWS-orange?style=for-the-badge\&logo=amazonaws)
![Git](https://img.shields.io/badge/Git-red?style=for-the-badge\&logo=git)

---

## 📊 GitHub Stats

![Stats](https://github-readme-stats.vercel.app/api?username=IrfanAhmed8\&show_icons=true\&theme=tokyonight)
![Streak](https://streak-stats.demolab.com?user=IrfanAhmed8\&theme=tokyonight)

---

## 🔥 Featured Projects

### 💻 Code-Mate (Real-Time Collaborative Coding Platform)

* ⚡ CRDT-based collaboration using Yjs
* 🌐 WebSocket + WebRTC for low-latency sync
* 🚀 Scalable backend for concurrent users

---

### 🤖 ResumeInQlik (AI Resume Generator)

* 🧠 Built using Spring Boot + React
* ✨ Integrated Gemini API for dynamic resumes
* ⏱️ Reduced resume creation time by **80%**

---

### 🏋️ AI Fitness Tracker (Microservices App)

* 🔄 Kafka-based async architecture
* 🐳 Dockerized services
* 📊 MySQL + MongoDB hybrid data handling

---

## 💼 Experience

**Full Stack Developer Intern @ DekNek3D**

* Built scalable REST APIs using Node.js & Express
* Improved system efficiency by **30%**
* Optimized backend pipelines for 3D file processing

---

## 🧩 Achievements

* 🏆 300+ LeetCode Problems Solved
* 🔥 100-Day Coding Streak
* 🎮 Built Unity Game: *Snow and Water*
* 📜 Google GenAI Study Jam Participant

---

## 🌐 Connect With Me

* 💼 LinkedIn: https://linkedin.com/in/irfanahmedjafri
* 🧠 LeetCode: https://leetcode.com/u/irfan66
* 💻 GitHub: https://github.com/IrfanAhmed8

---

## ⚡ Fun Fact

I enjoy optimizing systems almost as much as solving DSA problems ☕

---
